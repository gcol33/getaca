...
Two established packages solve neighbouring problems, and either may be the
better fit depending on which problem you have.
...
`package / name / version`, the declaration ships inside the installed package,
and resolution, verification, offline behaviour and retention are the same for
every declaring package because there is one engine.
