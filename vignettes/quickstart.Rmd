...
`"wfo"` and never collide, in the cache or anywhere else.
...
`getaca("wfo")` asks for a name, and the registry says which record that name
means. When a package offers several versions, it states the answer:
...
"which version of taxify's declaration was this" has an answer years later,
...
- `vignette("cache")` for the layout, verification schedule, locking and
  retention
- `vignette("failures")` for the six conditions and how to handle each
