...
A plain downloader reports eleven different situations the same way:
something went wrong with a URL. `getaca` separates them, because the
eleven have three different people who can act on them and five
...
All eleven inherit from `getaca_error`, which inherits from `error`, so
a handler for `getaca_error` catches everything the package raises and
nothing else.
...
Every mirror that failed answered 401 or 403. The resource is reachable
and the machine has a network; what is missing is permission. Telling
that user to connect to a network would send them looking in the wrong
place, which is the same reasoning that separates a truncation from an
outage.
...
response, because a server that never saw a credential and one that
rejected the credential it saw answer alike. The message says which of
the two happened, since “not set” and “set, and refused” call for
...
credential; the value is read at the moment of the request and never
stored.

This is the one condition whose `actor` depends on the situation. Where
the declaration names a credential, the person who can act is the user
holding it. Where a host refuses and the declaration names none, there
is no variable to set: either the resource became restricted after it
was declared or the location is wrong, and both are the declaring
package’s to fix, so `actor` is `"author"` and the message says so
instead of asking for a variable that does not exist.

To ask before fetching rather than after failing,
...
for how a package declares one.
...
The cached copy still matches its own record, and the declaration in
force has moved to different bytes for that version. Both are intact,
which is what separates this from a corrupt cache:
...
Resolving quietly to the copy already held would be the wrong answer: it
makes one machine keep bytes every other machine has stopped using, and
the disagreement stays invisible for as long as that cache survives.
`verify = TRUE` asks the same question, so a forced re-hash cannot
confirm bytes against a declaration they no longer match either.

A version reaches this state when a package upgrade brings an edited
record for a version this machine has already fetched. A remote registry
or a pin file trying the same thing is stopped earlier, at
...
A remote registry arriving without a signature this package’s own keys
accept. Raised only where the bundled registry declares `keys`; a
package declaring none never fetches a signature and never meets this:
...
The same condition covers a signature from a key the package does not
declare, one whose expiry has passed, and one covering a declaration
state older than the installed registry, which is what stops an old
genuine registry being replayed in place of the current one.

A host that cannot be reached is a different situation and gets the
other answer: resolution falls back to the bundled declaration with a
message. A registry that arrives and fails verification stops instead. A
registry arriving without its signature is the second, since one host
serves both, and treating a missing signature as an outage would let
whoever serves the file decide whether the check runs at all.

See
[`vignette("policies")`](https://gillescolling.com/getaca/articles/policies.md)
for how a package declares keys and signs what it publishes.
